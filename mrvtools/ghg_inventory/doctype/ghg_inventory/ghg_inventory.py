...
class GHGInventory(Document):
	...
	def get_data(self,table_name):
		get_doc = frappe.db.sql(
			"""SELECT * FROM `tabDirect and Indirect Managed Soils Master List` WHERE table_name = %s""",
			(table_name,),
			as_dict=1,
		)
		return get_doc
	...
	def get_livestock_details(self):
		# order_by=idx asc matches the on-form row ordering of this child table.
		# Without an explicit order_by, v16 falls back to `creation DESC` — which
		# is the wrong visual order for a child-table listing.
