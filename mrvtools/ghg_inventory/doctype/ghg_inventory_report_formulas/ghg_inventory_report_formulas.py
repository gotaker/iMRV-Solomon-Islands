...
class GHGInventoryReportFormulas(Document):
	# @frappe.whitelist()
	def on_update(self):
		tablefields=[]
		...
		for i in document_name:
			...
			if i.sector == '1. Energy':
				...
				frappe.enqueue(ippu_calculation, doc=doc, doc_name=doc_name, tablefields=tablefields)

			elif i.sector == '3. Agriculture':
				frappe.enqueue(agri_calculation, doc=doc, doc_name=doc_name, tablefields=tablefields)

			elif i.sector == '4. LAND USE, LAND-USE CHANGE AND FORESTRY':
				frappe.enqueue(land_calculation, doc=doc, doc_name=doc_name, tablefields=tablefields)

			elif i.sector == '5. Waste':
				frappe.enqueue(waste_calculation, doc=doc, doc_name=doc_name, tablefields=tablefields)

			else:
				frappe.enqueue(other_calculation, doc=doc, doc_name=doc_name, tablefields=tablefields)
