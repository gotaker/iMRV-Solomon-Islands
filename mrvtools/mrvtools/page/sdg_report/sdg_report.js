...
class SDG {
	constructor(parent) {
		...
		this.page.sidebar.html(
			`<ul class="standard-sidebar sdg_report-sidebar overlay-sidebar"></ul>`
		);
	...
	set_default_secondary_action() {
		...
		this.download_button = this.page.set_secondary_action('Download', () => {
			...
			frappe.call('mrvtools.mrvtools.page.sdg_report.sdg_report.execute',{
				...
				.then((r) => {
					frappe.call('mrvtools.mrvtools.page.sdg_report.sdg_report.download_excel',{
						columns:r.message[0],
						data:r.message[1]
					}).then((i) =>{
		...
		let key_sub_sector = frappe.ui.form.make_control({
			df: {
				...
				get_query: function () {
					return {
						"filters": {
							"key_sector": ["in", [`${frappe.sdg_report.key_sector}`]]
						}
	...
	get_total_sdg_report() {
		frappe.call('mrvtools.mrvtools.page.sdg_report.sdg_report.get_total_sdg_report_data',{
			...
			.then((r) => {
				...
				frappe.utils.make_chart(".totalsdg_report-graph", custom_options);
			});
