...
Phase 1 shipped 4 journey templates + 10 regression scenarios derived from memory entries (`reference_csrf_retry_interceptor.md`, `reference_drawer_perm_filter.md`, `reference_v16_breadcrumb_selector.md`, etc.); the regression suite has since grown to 40+ YAMLs covering audit findings — every new memory entry describing a runtime invariant should land a paired scenario in [bench/scenarios/regression/](bench/scenarios/regression/) (per `feedback_translate_memory_to_assertions.md`).
...
**Tracking Report PDF export.** All 8 desk Tracking Reports (Mitigation, Adaptation, Finance, NDC, SDG, MRV per-project, GHG Inventory Gas-Wise, GHG Year-Wise) share one Jinja template at [mrvtools/templates/pdf/tracking_report.html](mrvtools/templates/pdf/tracking_report.html) and one helper at [mrvtools/mrvtools/pdf_export.py](mrvtools/mrvtools/pdf_export.py). Each report page exposes a tiny `download_pdf(filter_args)` adapter that calls its existing `getColumns()` / `getData()` / `get_chart()` / `get_pie_chart()`, normalises the shapes (tuples vs dicts, ghg-inventory's `indent`, sdg's html-image sentinels, mrv's per-project sub-sections), then hands off to `render_tracking_report_pdf(...)`. Charts are **re-rendered server-side** from the same `chart_data` the frappe-charts JS receives — two zero-dependency SVG primitives (bar + donut) live in `pdf_export.py`. Don't try to reuse the live JS charts: the server pass deliberately avoids the audit's "undefined" tooltips, Y-axis clip, and single-segment pies reading as solid disks. Output uses the Forest-and-Sage editorial palette flattened to print-safe hex (no transparency/blur). Reports with >12 columns auto-flip to A4 landscape via `table-layout: fixed` + ellipsis. Render path is wkhtmltopdf, baked into the Docker image — local dev needs `wkhtmltopdf` on `$PATH` for the button to succeed.
