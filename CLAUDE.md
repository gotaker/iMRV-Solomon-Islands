...
[frontend/src/main.js](frontend/src/main.js) wires Frappe-UI with `frappeRequest` as the resource fetcher and registers `Button`/`Card`/`Input` globally. If a Frappe-UI component looks unresolved, it needs registering here. Scroll-reveal animations use a shared `useReveal()` composable at [frontend/src/composables/useReveal.js](frontend/src/composables/useReveal.js); pages tag elements with `data-reveal` and the composable adds `is-revealed` synchronously for in-viewport elements on mount, observing the rest. Replaced 8 page-local `IntersectionObserver`s whose `threshold:0.12` + negative bottom `rootMargin` left short headlines stuck at `opacity:0`. The `aos` package is still in `package.json` but no longer imported. Stale CSRF tokens (e.g. desk session expired mid-tab) trigger an axios response interceptor in `main.js` that refetches via `/api/method/frappe.sessions.get_csrf_token` (with `GET /` + cookie-parse fallback) and retries once; per-request `__csrfRetried` flag prevents loops.
